...
If MySQL native partitioning is not used, create archive tables and scheduled
retention jobs:
...
CREATE TABLE operations_hourly_transfer_metrics (
    ...
    created_at DATETIME(3) NOT NULL,
    UNIQUE KEY uk_hourly_metric (bucket_start, source_bank, destination_bank, status)
);
```

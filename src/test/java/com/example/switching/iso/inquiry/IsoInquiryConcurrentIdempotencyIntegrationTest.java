 ...
 * do not create two iso_inquiries rows.  The race loser must catch the UNIQUE
 ...
 *            exactly 1 row in iso_inquiries
 */
class IsoInquiryConcurrentIdempotencyIntegrationTest extends AbstractIntegrationTest {
    ...
    void sequentialRetry_sameMsgId_returnsSameInquiryRef() throws Exception {
        ...
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM iso_inquiries WHERE channel_id = 'ISO20022_XML' AND message_id = ?",
                Integer.class, msgId);
        assertEquals(1, count, "Exactly 1 iso_inquiries row must exist for the message_id");
    }

    // ─────────────────────────────────────────────────────────────────────────
    // TC-CI-002  Concurrent race — both threads MTCH, 1 row in iso_inquiries
    // ─────────────────────────────────────────────────────────────────────────
    @Test
    void concurrentSameMsgId_bothGetMtch_exactlyOneRowCreated() throws Exception {
        ...
        assertEquals(1, count,
                "Exactly 1 iso_inquiries row must exist even when two threads raced on the same MsgId");
    }

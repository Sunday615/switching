 ...
 * {@code next_retry_at} timestamp advances by the expected backoff delay:
 *
 *   attempt 1 → +30 s   (±5 s tolerance)
 *   attempt 2 → +120 s  (±10 s tolerance)
 *   attempt 3 → +600 s  (±15 s tolerance)
 *
...
class OutboxBackoffIntegrationTest extends AbstractIntegrationTest {
    ...
    void attempt1_setsNextRetryAt_30seconds() {
        ...
        Map<String, Object> row = jdbcTemplate.queryForMap(
                "SELECT status, retry_count, next_retry_at FROM outbox_events WHERE id = ?",
                outboxEventId);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // TC-BO-002  Attempt 2 → next_retry_at ≈ now + 120 s (2 min)
    // ─────────────────────────────────────────────────────────────────────────
    @Test
    void attempt2_setsNextRetryAt_2minutes() {
        ...
        // next_retry_at should be in [before+110s, after+130s]
        assertTrue(nextRetry.isAfter(before.plusSeconds(110)),
                "next_retry_at should be at least 110 s in the future (2 min - 10 s tolerance)");
        assertTrue(nextRetry.isBefore(after.plusSeconds(130)),
                "next_retry_at should be at most 130 s in the future");
    }

    // ─────────────────────────────────────────────────────────────────────────
    // TC-BO-003  Attempt 3 (default max-retry=3) → event goes FAILED,
    //            next_retry_at is null (no more retries)
    // ─────────────────────────────────────────────────────────────────────────
    @Test
    void attempt3_atMaxRetry_setsStatusFailed_noNextRetry() {
        String transferRef  = "BO-TXN-3-" + System.nanoTime();
        // retry_count=2 means this is the 3rd attempt, which hits maxRetry=3
        long outboxEventId  = insertPendingOutboxEvent(transferRef, 2);
    ...
    void pendingEventWithFutureNextRetryAt_isNotClaimedEarly() {
        ...
        jdbcTemplate.update(
                "UPDATE outbox_events SET next_retry_at = ? WHERE id = ?",
    ...
    private long insertPendingOutboxEvent(String transferRef, int initialRetryCount) {
        ...
        jdbcTemplate.update(
                ...
                INSERT INTO transfers (
                    transfer_ref, client_transfer_id, idempotency_key,
                    source_bank_code, source_account_no,
                    destination_bank_code, destination_account_no,
                    amount, currency, channel_id, status,
                    created_at, updated_at
                ) VALUES (?, ?, ?, ?, '010100000001', ?, '020200000003',
                          250000.00, 'LAK', 'API', 'RECEIVED', ?, ?)
                ON DUPLICATE KEY UPDATE updated_at = VALUES(updated_at)
                """,
                transferRef, transferRef, transferRef,
                SOURCE_BANK, DEST_BANK, now, now);

        // messageType lives in outbox_events.message_type column, not the JSON payload.
        ...
        jdbcTemplate.update(
                ...
                INSERT INTO outbox_events (
                    transfer_ref, status, message_type, payload,
        ...
        return jdbcTemplate.queryForObject(
                "SELECT id FROM outbox_events WHERE transfer_ref = ? ORDER BY id DESC LIMIT 1",
    ...
    private LocalDateTime toLocalDateTime(Object value) {
        ...
        // MySQL JDBC returns java.sql.Timestamp
